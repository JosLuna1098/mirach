...
- **LLM:** [OpenCode CLI](https://opencode.ai) **2.0 or newer** — your choice of
  model, with the full tool ecosystem (bash, file I/O, custom commands).
...
Install to a different directory (default is `~/mirach`):

```bash
curl -fsSL https://raw.githubusercontent.com/JosLuna1098/mirach/master/bootstrap.sh | MIRACH_DIR=~/Projects/mirach bash
```
...
3. Run `opencode auth` to configure your LLM provider if you haven't already
   (Mirach requires opencode >= 2.0 — check with `opencode --version`).
...
Mirach supervises an `opencode serve` subprocess and drives its v2 HTTP API, so
first check the binary is recent enough and that a model is reachable:

```bash
opencode --version          # must be 2.0 or newer
opencode run "hello"
```
